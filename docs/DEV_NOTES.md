...
3. ~~**Orchestrateur fait du code** même si l'agent code est restreint (routage qui fuit).~~ **CORRIGÉ (2026-07-02)** : deux fuites fermées dans `core/swarm/engine.py` — (a) `execute_bash_command`/`list_ssh_hosts` n'étaient JAMAIS retirés à l'orchestrateur (`_orch_keep` inconditionnel) → il codait via bash au lieu de déléguer ; désormais conservés UNIQUEMENT si un hôte SSH est configuré. (b) Zone AMBIGUË du routeur (2 spécialistes proches → aucune restriction) : `agent_router` mémorise ses top-candidats et le moteur injecte un indice DOUX dans le préambule (« relève de X ou Y → délègue ») sans retirer d'outils. Tests : `tests/test_route_no_leak.py`.
